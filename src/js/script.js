document.addEventListener('DOMContentLoaded', function () {
	...
	function initSimpleBar() {
		Array.prototype.forEach.call(
			document.querySelectorAll('.custom-simplebar'),
			el => new SimpleBar(el)
		)
	}

	function initSwiper() {
		const container = document.querySelector('.articles-slider')

		if (!container) return

		const parent = container.closest('.articles')

		const nextEl = parent?.querySelector('.swiper-button-next')
		const prevEl = parent?.querySelector('.swiper-button-prev')

		const articlesSwiper = new Swiper(container, {
			loop: true,
			spaceBetween: 16,
			slidesPerView: 'auto',
